=== Responsive Media Gallery ===
Contributors: DigitalCourt
Donate link: 
Tags: 
Requires at least: 3.6
Tested up to: 3.8
Stable tag: trunk
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Replace the default Wordpress gallery with a responsive and touch friendly Media slideshow.

== Description ==

Now you can create beautiful responsive gallery slideshows within minutes

Leveraging the new built in gallery user interface in WordPress, you can now drag and drop images to create perfectly responsive gallery slideshows with ease

Here’s why responsive media gallery will make your life easier:

- Uses the new easy to use drag and drop media gallery functionally in WordPress

- Responsive for any screen size

- Touch friendly interface

- Minimal interface elements to focus on gallery

- Zero configuration makes it easy to use

How to use:

1. Click “Add Media” in any page or post

2. Select “Create Gallery”

3. Choose your images

4. “Create a new Gallery”

Responsive media gallery will be the last slideshow plugin you’ll ever need!

See a preview here:
Part of the [Marketer WordPress business theme](http://themersclub.com/marketer/portfolio/paradise/) <- see a live preview of the responsive media gallery here (resize the screen to see the awesome responsiveness!)

== Installation ==

1. Upload `responsive-media-gallery` to the `/wp-content/plugins/` directory
1. Activate the plugin through the 'Plugins' menu in WordPress
1. Go to widgets or add via shortcode

== Screenshots ==

1. Responsive Media Gallery live
2. How to add images to your gallery

== Changelog ==

Initial Release

== Upgrade notice ==

Upgrade as needed :)
